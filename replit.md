...
- TV Dashboard System: Multi-department TV displays with configurable KPIs and video playback per department. Admin config page for dashboard/KPI/video management. Public routes at `/tv-dashboard/:id` (no auth required). Dark theme full-screen display with auto-refresh. Features: `showVideo` toggle per dashboard, configurable `videoPosition` (7 layouts), `kpiRotationSeconds`, `kpiTransitionStyle` (fade/slide-left/slide-up/zoom), animated count-up KPI values, subtle glow/pulse effects on cards. Video playback is unaffected by KPI page transitions (VideoPanel/KpiGrid are React.memo components).
