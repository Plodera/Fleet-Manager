...
- IT Operations Dashboard & Network Monitor: Full-screen dark TV display for IT infrastructure monitoring. ICMP ping monitoring runs every 30s for all active hosts. Config page at `/it-monitor-config` (admin only). Public display at `/it-dashboard`. Features: Internet link status pills (UP/DOWN with response time), IP camera grid (online/offline), animated count-up KPI cards with glow/pulse effects, live clock, fullscreen mode. KPIs support daily and monthly values with data entry UI. Host types: internet_link, camera, other. Bilingual (EN/PT). Database tables: `it_monitored_hosts`, `it_host_status`, `it_kpis`, `it_kpi_values`. SQL migration: `sql/add_it_monitor.sql`. Ping service: `server/itMonitor.ts` (starts automatically on server startup).
