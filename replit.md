...
- GLPI Integration: Automatically pulls ticket counts from a GLPI helpdesk instance into IT Dashboard KPIs. Config via "GLPI" tab in IT Monitor Config — enter GLPI base URL, Application Token, User API Token, sync interval (minutes), and enable toggle. "Sync Now" button for manual trigger. Auto-creates 3 KPIs: `glpi_open_tickets` (active tickets in statuses New/Processing/Pending), `glpi_new_today` (tickets opened today), `glpi_resolved_today` (tickets resolved/closed today). Sync service starts on server startup and reschedules based on configured interval. Database table: `glpi_settings`. SQL migration: `sql/add_glpi_settings.sql`. Sync service: `server/glpiSync.ts`.
