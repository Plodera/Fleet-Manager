...
type Rule = { id: number; entityType: string; triggerType: string; thresholdDays: number | null; sendEmail: boolean; sendInApp: boolean; preferredTime: string; scheduleTimezone: string; timesPerDay: number; isActive: boolean; recipients: Recipient[] };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <Tabs defaultValue={canLicenses ? "licenses" : "documents"} className="space-y-5">
        ...
        {isAdmin && <TabsContent value="rules" className="space-y-4">
          ...
          <Card><CardContent className="space-y-3 pt-6">{rules.map(rule => <div key={rule.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-4"><div><p className="font-medium">{le.entityTypes[rule.entityType as keyof typeof le.entityTypes]} · {rule.triggerType === "expired" ? le.whenExpired : le.daysBefore.replace("{days}", String(rule.thresholdDays))}</p><p className="mt-1 text-sm text-muted-foreground">{rule.recipients.length} {le.recipients.toLowerCase()} · {rule.sendEmail && le.email} {rule.sendInApp && le.inApp}</p><p className="mt-1 text-xs text-muted-foreground">{le.scheduleSummary.replace("{time}", rule.preferredTime).replace("{count}", String(rule.timesPerDay)).replace("{timezone}", rule.scheduleTimezone)}</p></div><div className="flex flex-wrap items-center gap-2"><Badge variant={rule.isActive ? "default" : "secondary"}>{rule.isActive ? le.active : le.inactive}</Badge><Button size="sm" variant="outline" disabled={!rule.sendEmail || testRule.isPending} onClick={() => testRule.mutate(rule.id)}><MailCheck className="mr-1.5 w-3.5 h-3.5" />{testRule.isPending && testRule.variables === rule.id ? le.testing : le.testNotification}</Button><Button size="sm" variant="outline" aria-label={t.buttons.edit} onClick={() => openRule(rule)}><Pencil className="w-3.5 h-3.5" /></Button><Button size="sm" variant="outline" aria-label={t.buttons.delete} onClick={() => deleteRule.mutate(rule.id)}><Trash2 className="w-3.5 h-3.5" /></Button></div></div>)}{rules.length === 0 && <p className="py-8 text-center text-muted-foreground">{le.noRules}</p>}</CardContent></Card>
        </TabsContent>}
