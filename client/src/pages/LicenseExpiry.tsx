...
import { Bell, CalendarClock, Check, ChevronDown, Download, Eye, FileSpreadsheet, MailCheck, Pencil, Plus, Search, ShieldCheck, Trash2, Upload, UserCheck, X } from "lucide-react";
...
type DeliveryAttempt = { id: number; deliveryType: "scheduled" | "test"; channel: string; recipientLabel: string | null; success: boolean; error: string | null; attemptedAt: string };
type Rule = { id: number; entityType: string; triggerType: string; thresholdDays: number | null; sendEmail: boolean; sendInApp: boolean; preferredTime?: string; scheduleTimezone?: string; timesPerDay?: number; isActive: boolean; recipients?: Recipient[]; nextDelivery?: { date: string; time: string; timezone: string } | null; recentDeliveries?: DeliveryAttempt[] };
...
const blankRule = { entityType: "vehicle_ownership", triggerType: "expiry_approaching", thresholdDays: "90", sendEmail: true, sendInApp: true, preferredTime: "09:00", scheduleTimezone: "Africa/Lagos", timesPerDay: "1", userIds: [] as number[], extraEmails: "", isActive: true };
...
export default function LicenseExpiry() {
  ...
  const { data: rules = [] } = useQuery<Rule[]>({
    queryKey: ["/api/expiry-notification-rules"],
    enabled: isAdmin,
  ...
  const ruleMutation = useMutation({
    mutationFn: async () => {
      ...
      return apiRequest(editingRule ? "PUT" : "POST", editingRule ? `/api/expiry-notification-rules/${editingRule.id}` : "/api/expiry-notification-rules", {
        ...
        sendEmail: ruleForm.sendEmail, sendInApp: ruleForm.sendInApp,
        preferredTime: ruleForm.preferredTime, scheduleTimezone: ruleForm.scheduleTimezone,
        timesPerDay: Number(ruleForm.timesPerDay), isActive: ruleForm.isActive, recipients,
      });
    },
  ...
  const testRule = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest("POST", `/api/expiry-notification-rules/${id}/test`);
      return response.json() as Promise<{ sentCount: number; failedCount: number }>;
    },
    onSuccess: result => toast({ title: le.testSent, description: le.testSentDescription.replace("{count}", String(result.sentCount)) }),
    onError: (error: Error) => toast({ title: le.testFailed, description: error.message, variant: "destructive" }),
    onSettled: () => queryClient.invalidateQueries({ queryKey: ["/api/expiry-notification-rules"] }),
  });
  ...
  const openRule = (rule?: Rule) => {
    ...
    setRuleForm(rule ? {
      ...
      preferredTime: rule.preferredTime || "09:00", scheduleTimezone: rule.scheduleTimezone || "Africa/Lagos",
      timesPerDay: String(rule.timesPerDay || 1),
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <Tabs defaultValue={canLicenses ? "licenses" : "documents"} className="space-y-5">
        ...
        {isAdmin && <TabsContent value="rules" className="space-y-4">
          <div className="flex justify-end"><Button onClick={() => openRule()}><Plus className="mr-2 w-4 h-4" />{le.addRule}</Button></div>
            <Card><CardContent className="space-y-3 pt-6">{rules.map(rule => <div key={rule.id} className="rounded-lg border p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-medium">{le.entityTypes[rule.entityType as keyof typeof le.entityTypes]} · {rule.triggerType === "expired" ? le.whenExpired : le.daysBefore.replace("{days}", String(rule.thresholdDays))}</p><p className="mt-1 text-sm text-muted-foreground">{(rule.recipients ?? []).length} {le.recipients.toLowerCase()} · {rule.sendEmail && le.email} {rule.sendInApp && le.inApp}</p><p className="mt-1 text-xs text-muted-foreground">{le.scheduleSummary.replace("{time}", rule.preferredTime || "09:00").replace("{count}", String(rule.timesPerDay || 1)).replace("{timezone}", rule.scheduleTimezone || "Africa/Lagos")}</p><p className="mt-2 text-sm font-medium text-primary">{rule.nextDelivery ? le.nextDelivery.replace("{date}", rule.nextDelivery.date).replace("{time}", rule.nextDelivery.time).replace("{timezone}", rule.nextDelivery.timezone) : !rule.isActive ? le.noNextDelivery : le.emailDeliveryDisabled}</p></div><div className="flex flex-wrap items-center gap-2"><Badge variant={rule.isActive ? "default" : "secondary"}>{rule.isActive ? le.active : le.inactive}</Badge><Button size="sm" variant="outline" disabled={!rule.sendEmail || testRule.isPending} onClick={() => testRule.mutate(rule.id)}><MailCheck className="mr-1.5 w-3.5 h-3.5" />{testRule.isPending && testRule.variables === rule.id ? le.testing : le.testNotification}</Button><Button size="sm" variant="outline" aria-label={t.buttons.edit} onClick={() => openRule(rule)}><Pencil className="w-3.5 h-3.5" /></Button><Button size="sm" variant="outline" aria-label={t.buttons.delete} onClick={() => deleteRule.mutate(rule.id)}><Trash2 className="w-3.5 h-3.5" /></Button></div></div>{(rule.recentDeliveries ?? []).length > 0 && <div className="mt-4 border-t pt-3"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{le.recentDeliveries}</p><div className="space-y-2">{(rule.recentDeliveries ?? []).map(delivery => <div key={delivery.id} className="flex flex-wrap items-start justify-between gap-2 text-xs"><span><Badge variant={delivery.success ? "default" : "destructive"}>{delivery.success ? le.deliverySucceeded : le.deliveryFailed}</Badge> <span className="ml-1 text-muted-foreground">{delivery.recipientLabel || le.recipientUnavailable} · {delivery.deliveryType === "test" ? le.testDelivery : le.scheduledDelivery} · {new Date(delivery.attemptedAt).toLocaleString()}</span></span>{delivery.error && <span className="max-w-xl text-destructive">{delivery.error}</span>}</div>)}</div></div>}</div>)}{rules.length === 0 && <p className="py-8 text-center text-muted-foreground">{le.noRules}</p>}</CardContent></Card>
        </TabsContent>}
      ...
      <Dialog open={ruleDialog} onOpenChange={setRuleDialog}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editingRule ? le.editRule : le.addRule}</DialogTitle></DialogHeader><div className="space-y-4"><div><Label>{le.appliesTo}</Label><Select value={ruleForm.entityType} onValueChange={entityType => setRuleForm({ ...ruleForm, entityType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(le.entityTypes).map(([value, label]) => <SelectItem key={value} value={value}>{label as string}</SelectItem>)}</SelectContent></Select></div><div><Label>{le.trigger}</Label><Select value={ruleForm.triggerType} onValueChange={triggerType => setRuleForm({ ...ruleForm, triggerType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="expiry_approaching">{le.daysBefore.replace("{days}", "")}</SelectItem><SelectItem value="expired">{le.whenExpired}</SelectItem></SelectContent></Select></div>{ruleForm.triggerType === "expiry_approaching" && <div className="space-y-2"><Label>{le.reminderDays}</Label><Input min="0" type="number" value={ruleForm.thresholdDays} onChange={event => setRuleForm({ ...ruleForm, thresholdDays: event.target.value })} /><div className="flex flex-wrap gap-2">{[90, 60, 30, 7].map(days => <Button key={days} type="button" size="sm" variant={ruleForm.thresholdDays === String(days) ? "default" : "outline"} onClick={() => setRuleForm({ ...ruleForm, thresholdDays: String(days) })}>{days}</Button>)}</div></div>}<div className="grid gap-4 rounded-lg border p-4 sm:grid-cols-3"><div><Label>{le.preferredTime}</Label><Input type="time" value={ruleForm.preferredTime} onChange={event => setRuleForm({ ...ruleForm, preferredTime: event.target.value })} /></div><div><Label>{le.timesPerDay}</Label><Input type="number" min="1" max="24" value={ruleForm.timesPerDay} onChange={event => setRuleForm({ ...ruleForm, timesPerDay: event.target.value })} /></div><div><Label>{le.timezone}</Label><Select value={ruleForm.scheduleTimezone} onValueChange={scheduleTimezone => setRuleForm({ ...ruleForm, scheduleTimezone })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Africa/Lagos">Africa/Lagos</SelectItem><SelectItem value="Africa/Luanda">Africa/Luanda</SelectItem><SelectItem value="Europe/Lisbon">Europe/Lisbon</SelectItem><SelectItem value="UTC">UTC</SelectItem></SelectContent></Select></div><p className="text-xs text-muted-foreground sm:col-span-3">{le.scheduleHint}</p></div><div className="space-y-2"><Label>{le.systemRecipients}</Label>{users.map(appUser => <label key={appUser.id} className="flex items-center gap-2 text-sm"><Checkbox checked={ruleForm.userIds.includes(appUser.id)} onCheckedChange={checked => setRuleForm({ ...ruleForm, userIds: checked ? [...ruleForm.userIds, appUser.id] : ruleForm.userIds.filter(id => id !== appUser.id) })} />{appUser.fullName}{appUser.email && <span className="text-muted-foreground">({appUser.email})</span>}</label>)}</div><div><Label>{le.extraEmails}</Label><Input value={ruleForm.extraEmails} onChange={event => setRuleForm({ ...ruleForm, extraEmails: event.target.value })} placeholder="ops@example.com, fleet@example.com" /><p className="mt-1 text-xs text-muted-foreground">{le.mailboxDeduplicationHint}</p></div><div className="flex items-center gap-2"><Switch checked={ruleForm.sendEmail} onCheckedChange={sendEmail => setRuleForm({ ...ruleForm, sendEmail })} /><Label>{le.email}</Label></div><div className="flex items-center gap-2"><Switch checked={ruleForm.sendInApp} onCheckedChange={sendInApp => setRuleForm({ ...ruleForm, sendInApp })} /><Label>{le.inApp}</Label></div><div className="flex items-center gap-2"><Switch checked={ruleForm.isActive} onCheckedChange={isActive => setRuleForm({ ...ruleForm, isActive })} /><Label>{le.active}</Label></div></div><DialogFooter><Button variant="outline" onClick={() => setRuleDialog(false)}>{t.buttons.cancel}</Button><Button onClick={() => ruleMutation.mutate()} disabled={(ruleForm.userIds.length === 0 && !ruleForm.extraEmails.trim()) || !ruleForm.preferredTime || Number(ruleForm.timesPerDay) < 1 || Number(ruleForm.timesPerDay) > 24 || ruleMutation.isPending}>{t.buttons.save}</Button></DialogFooter></DialogContent></Dialog>
